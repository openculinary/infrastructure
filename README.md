...
We recommend adding the configuration settings to `/etc/sysctl.d/50-reciperadar.conf`; the settings are:
...
```
# Keyrings
wget -qO - https://artifacts.opensearch.org/publickeys/opensearch-release.pgp | gpg --dearmor --batch --yes --output /etc/apt/keyrings/opensearch-release-keyring.gpg
wget -qO - https://download.opensuse.org/repositories/isv:/cri-o:/stable:/v1.34/deb/Release.key | gpg --dearmor --batch --yes --output /etc/apt/keyrings/cri-o-apt-keyring.gpg
...
The web server we use to handle TLS connections, `haproxy`, is configured to read the `live` certificate and private key files directly from there, by referencing them from a [certificate store (`crt-store`)](https://github.com/haproxy/haproxy/blob/d236b43da7149f5147474c03fd54fada0615ce91/doc/configuration.txt#L4782-L4867) configuration entry.
...
```bash
$ certbot revoke --reason superseded --cert-path /etc/letsencrypt/archive/reciperadar.com/cert_.pem
```
