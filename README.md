...
```
pacman -S elasticsearch postgresql rabbitmq haproxy cri-o kubeadm kubectl kubelet
...
```
vim /usr/lib/sysctl.d/99-sysctl.conf
...
...
```
for service in systemd-networkd elasticsearch postgresql rabbitmq squid haproxy crio kubelet;
do
...
```
# Deprecated, but required until kubeadm init cgroup driver detection works
